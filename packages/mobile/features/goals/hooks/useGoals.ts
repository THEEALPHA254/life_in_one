// Mirrors packages/web/src/features/goals/hooks/useGoals.ts — same TanStack
// keys via @lio/core. Status changes are optimistically patched across every
// cached year so the UI updates instantly.
          ...
          rows.map((g) =>
            g.id === id
              ? {
                  ...g,
                  status,
                  achieved_at: status === "achieved" ? new Date().toISOString() : g.achieved_at,
                }
              : g,
          ),
        );
