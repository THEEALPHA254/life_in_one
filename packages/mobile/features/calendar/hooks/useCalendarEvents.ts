// Mirrors packages/web/src/features/calendar/hooks/useCalendarEvents.ts —
// same TanStack keys via @lio/core so cache is coordinated across web + mobile.
// Push-to-Google runs after local write iff a google account is cached.
...
async function pushToGoogle(action: "push_local" | "delete", eventId: string): Promise<void> {
...
export function useEventsInRange(fromIso: string, toIso: string) {
  return useQuery({
    ...
    queryFn: async () =>
      (await api.calendar.listEventsInRange(supabase, fromIso, toIso)) as CalendarEventRow[],
...
export function useDeleteEvent() {
  ...
  return useMutation({
    mutationFn: async (id: string) => {
      if (hasGoogleAccount(qc)) {
        // Edge fn does both Google + local delete atomically.
