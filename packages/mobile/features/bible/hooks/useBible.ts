// Mirrors packages/web/src/features/bible/hooks/useBible.ts — same TanStack
// keys via @lio/core. bibleConfig reads EXPO_PUBLIC_* env; when unset, the
// integration falls back to bible-api.com (KJV, no API key required).
...
const bibleConfig = {
  apiKey: process.env.EXPO_PUBLIC_BIBLE_API_KEY || undefined,
  baseUrl: process.env.EXPO_PUBLIC_BIBLE_API_BASE || undefined,
  bibleId: process.env.EXPO_PUBLIC_BIBLE_ID || undefined,
};
...
// Autosave: patches note cache in place; never invalidates the record's own
// query (would race with in-flight typing, same trap as Journal editor).
...
export function useAddVerse() {
  ...
  return useMutation({
    mutationFn: async ({ noteId, reference }: { noteId: string; reference: string }) => {
      ...
      if (!parsed) {
        throw new Error(`Couldn't parse "${reference}". Try "John 3:16" or "John 3:16-18".`);
      }
      const range = parsed.verse_end
