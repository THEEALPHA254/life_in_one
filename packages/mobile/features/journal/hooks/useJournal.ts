// Mirrors packages/web/src/features/journal/hooks/useJournal.ts — same TanStack
// keys via @lio/core so cache is coordinated across web + mobile.
...
export function useEntries(search: string) {
  return useQuery({
    ...
    queryFn: async () =>
      (await api.journal.listEntries(supabase, {
        limit: 50,
...
// Autosave: patches caches in place. Never invalidates the day-list or the
// entry query — a refetch mid-typing would race with in-flight edits.
